...
(async () => {
  ...
  try {
    for (const viewport of [{width:1440,height:1000},{width:390,height:844},{width:320,height:568}]) {
      ...
      await page.locator('.topbar img').evaluate(image => image.decode());
      await page.locator('.card-logo').evaluate(image => image.decode());
      await page.frameLocator('.site-frame').locator('body').waitFor({timeout:20000});

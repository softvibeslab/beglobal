...
6. **Mobile flyer legibility for Telegram/WhatsApp/IG** — for simple commercial flyers, prefer a 9:16/portrait single-column layout with large headlines and 3–5 short blocks. Avoid tiny disclaimers, dense paragraphs, and two-column cards unless the text is extremely short. After generating, verify with `vision_analyze`; if any text is cut off, descuadrado, too small, or outside margins, regenerate before sending. User may ask to remove “letras pequeñas”; keep only essential text and make the layout breathe.
7. **Iterative design edits** — when the user asks for copy/pricing/title changes, regenerate the final asset and run a visual check before final delivery. Preserve the latest approved design direction unless explicitly changed.
6. **Telegram/IG commercial flyers for this user** — prioritize legibility over density: one-column layout, large type, short lines, generous margins, no tiny footnotes/disclaimers, and no crowded bottom sections. After generating, verify with vision for cut-off/overlap/alignment before delivery; if text is clipped, reduce content or font size and regenerate.
6. **Flyer legibility before delivery** — for client-facing flyers, especially Telegram/WhatsApp/Instagram vertical assets, avoid tiny disclaimer text and crowded side-by-side pricing cards. Prefer a single-column layout with generous margins, shorter labels, and fitted font sizes. Before delivering, visually verify that no text is clipped at the right/bottom edge, no lines overlap card borders, and every price/CTA is fully readable on a phone screen. If clipping appears, reduce copy and rebuild rather than sending a flawed design.

...
(() => {
  ...
  function openHermesChat(prompt = "") {
    ...
    if (prompt) {
      chatFrame.contentWindow?.postMessage(
        { type: "beglobal-prefill", prompt },
        "https://chatbeglobal.softvibes.pro",
      );
    }
  }
  ...
  chatFrame.addEventListener("load", () => {
    if (pendingPrompt) {
      chatFrame.contentWindow?.postMessage(
        { type: "beglobal-prefill", prompt: pendingPrompt },
        "https://chatbeglobal.softvibes.pro",
      );
    }
    pendingPrompt = "";
  ...
  closeChatButton.addEventListener("click", closeHermesChat);
  chatBackdrop.addEventListener("click", closeHermesChat);
  ...
  async function initVoice() {
    try {
      ...
      if (
        !uuid.test(config.assistantId || "")
        || config.assistantIdConfirmed !== true
      ...
      status.textContent = "Canal de voz listo. Pulsa “Hablar” cuando quieras comenzar.";
    } catch {

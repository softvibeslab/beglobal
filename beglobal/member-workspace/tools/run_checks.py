...
def source_inventory():
    candidates = [HERE / "workspace.py", HERE / "test_workspace.py", HERE / "playwright.config.cjs", HERE / "package.json", HERE / "package-lock.json", HERE / "requirements.txt", HERE / "requirements.lock.txt"]
...
def main():
    ...
    results = {
        ...
        "backend": command("backend", [python, "-m", "unittest", "-v", "test_workspace.py"], HERE),
    ...
    stable = before == after
    local_pass = stable and baseline["unchanged"] and all(r["result"] == "PASS" for r in results.values())
    revision = digest(json.dumps(after, sort_keys=True, separators=(",", ":")).encode())
    report = {"recorded_at": datetime.now(timezone.utc).isoformat(), "scope": "SP-001 local fixture app, not product acceptance or production",

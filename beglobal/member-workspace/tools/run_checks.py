...
def source_inventory():
    candidates = [HERE / "workspace.py", HERE / "telegram_initdata.py", HERE / "test_workspace.py", HERE / "test_telegram.py", HERE / "test_link.py", HERE / "test_session.py", HERE / "playwright.config.cjs", HERE / "package.json", HERE / "package-lock.json", HERE / "requirements.txt", HERE / "requirements.lock.txt"]
...
def main():
    ...
    results = {
        ...
        "backend": command("backend", [python, "-m", "unittest", "-v", "test_workspace.py", "test_telegram.py", "test_link.py", "test_session.py"], HERE),
    ...
    report = {"recorded_at": datetime.now(timezone.utc).isoformat(), "scope": "SP-004 local session revoke and close-all, not product acceptance or production",

...
async function startSession() {
  input.disabled = true;
  send.disabled = true;
  status.textContent = "Iniciando sesión segura…";
...
async function submitMessage(message) {
  ...
  try {
    ...
    status.textContent = "Puedes volver a intentarlo.";
  } finally {
    input.disabled = false;
    send.disabled = false;
    input.focus();
...
function showStartupError() {
  status.textContent = "El chat no está disponible en este momento. Inténtalo más tarde.";
}
